                ...
                <div class="row">
                    <div style="align-items:center; justify-content:center; " class="row m-2 d-flex  text-center">
                        @forelse ($announcements as $announcement)
                            <div
                                class="card m-4 col-12 col-md-6"style="width: 24rem;  box-shadow: rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset; ">
                                <div class="card-body">
                                    ...
                                    <div style="display: flex; align-items:center; justify-content:end">
                                        <a href="" class="card-link ">
                                            @if (Auth::check() && Auth::user()->is_revisor == 1)
                                                <a href="{{ route('announcement.revision', $announcement->id) }}"
                                                    class="card-link ">
                                                    <button style="width:150px" type="submit"
                                                        class="btn btn-danger">{{ __('ui.rev2') }}</button>
                                                </a>
                                            @endif
                                            <a href="{{ route('dettaglio', compact('announcement')) }}"
                                                class="card-link ">
                                    ...
                                    <form style="margin-top: 12px"
                                        action="{{ route('cart.add', ['announcement' => $announcement->id]) }}"
                                        method="POST">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $announcement->id }}">
                                        <button style="background: #048cee;; width:220px;" type="submit"
                                            class="btn btn-primary">{{ __('ui.buy') }}</button>
                                    </form>
                                </div>
                            </div>
                        @empty
                            <div class="col-12">
                                <div class="alert alert-warning py-3 shadow">
                                    ...
                                    <p>{{ __('ui.newAdd') }}: <a href="{{ route('create') }}"
                                            style="color: #2414da">{{ __('ui.pubb') }}</a></p>
                                </div>
                            </div>
                        @endforelse
                        {{ $announcements->links() }}
